...
def update_line(arrL,arrR,plot):
    #print(arrL)
    #print(arrR)
        ...
        if object_countL >= object_threshold and object_countL < object_wall:
            if (GLOBAL_OBJ_L < 7):
                ...
                if (GLOBAL_OBJ_L > 3):
                    ...
                    plot.text(0, 200, 'Left object detected')
                    if (OBJ_CLOSE == 1):
                        print("BEEP")
            if (GLOBAL_WALL_L > 0):
            ...
            if (GLOBAL_WALL_L < 7):
                ...
                if (GLOBAL_WALL_L > 3):
                    ...
                    plot.text(0, 200, 'Left wall detected')
            if (GLOBAL_OBJ_L > 0):
                GLOBAL_OBJ_L = GLOBAL_OBJ_L - 1
        else:
            ...
            if (GLOBAL_WALL_L < 4 and GLOBAL_OBJ_L < 4):
                ...
                #Makes all the text on the plot invisible
        ...
        if object_countR >= object_threshold and object_countR < object_wall:
            ...
            if (GLOBAL_OBJ_R < 7):
                ...
                if (GLOBAL_OBJ_R > 3):
                    ...
                    plot.text(0, -200, 'Right object detected')
                    if (OBJ_CLOSE == 1):
                        print("BEEP")
            if (GLOBAL_WALL_R > 0):
            ...
            if (GLOBAL_WALL_R < 7):
                ...
                if (GLOBAL_WALL_R > 3):
                    ...
                    plot.text(0, -200, 'Right wall detected')
            if (GLOBAL_OBJ_R > 0):
                GLOBAL_OBJ_R = GLOBAL_OBJ_R - 1
        else:
            #print("\n")
...
def plotFn(frame):
    ...
    with open(os.path.join(path, currentFile), 'r') as csvfile:  # open in read-only mode
        ...
        '''
        Build array that will be used for plotting
        '''
        ...
        '''
        Build arrays that will be fed into object detection algorithm
        '''
    ...
    '''
    Pass data from above file read into the object detection algorithm
    '''
    update_line(cartesian_arrL,cartesian_arrR,ax)

'''
Animation of the data read from the CSV
'''
anim = animation.FuncAnimation(fig, plotFn, interval = 500)
plt.show()
